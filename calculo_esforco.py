...
st.caption("Baseado na Planilha de Cálculos DIS-NOR-014 — Rev. 2024")
...
# ─────────────────────────────────────────────────────────────────────────────
# WIDGET DE SELEÇÃO DE CABO (reutilizável)
# ─────────────────────────────────────────────────────────────────────────────

def widget_cabo(prefixo, label_cabecalho):
    """Renderiza seletor de tipo de rede + cabo e retorna a tração em daN."""
    st.markdown(f"**{label_cabecalho}**")
    tipo = st.selectbox("Tipo de rede", ["Convencional","Protegida","Compacta"], key=f"{prefixo}_tipo")
    tracao = 0.0
...
def widget_cabo_bt(prefixo, label_cabecalho):
    """Seletor de cabo BT (Convencional, Pré-Reunido ou CAZ/CAW)."""
    st.markdown(f"**{label_cabecalho}**")
    tipo = st.selectbox("Tipo de cabo", ["Convencional","Pré-Reunido","CAZ/CAW"], key=f"{prefixo}_tipo")
    tracao = 0.0

    if tipo == "Convencional":
        c1, c2 = st.columns(2)
        qtd  = c1.number_input("Qtd. cabos", 1, 10, 3, key=f"{prefixo}_qtd")
        cabo = c2.selectbox("Cabo BT", list(TRACAO_CABO.keys()), key=f"{prefixo}_cabo")
        tracao = float(TRACAO_CABO.get(cabo, 0)) * qtd
...
# ─────────────────────────────────────────────────────────────────────────────
# PAINEL COMPLETO DE UM NÍVEL (chegada + saída com cabos independentes)
# ─────────────────────────────────────────────────────────────────────────────

def painel_nivel(titulo, idx, alt_default, altura_util, altura_poste, is_bt=False):
    """
    Renderiza um nível completo com:
      - Cabo de CHEGADA (tipo + tração)
      - Ângulo de chegada
      - Tipo de SAÍDA: Fim de linha / Tangente (mesmo ou outro cabo) / Derivação
      - Cabo de SAÍDA (só se diferente) + ângulo de saída
    Retorna (rx_transf, ry_transf, mag_transf)
    """
    st.markdown(f"#### {titulo}")

    # Altura da estrutura
    alt_est = st.number_input(
        "Altura da estrutura (m)", 0.0, float(altura_poste),
    ...
    with col_sa:
        ...
        tipo_saida = st.radio(
            "O cabo...",
            ["Fim de linha (não sai nada)", "Sai na tangente — mesmo cabo", "Sai na tangente — cabo diferente", "Deriva em outro ângulo"],
        ...
        if tipo_saida == "Fim de linha (não sai nada)":
            ...
            st.info("Cabo reto com mesma seção: forças se cancelam → resultante ≈ 0.")
            ...
            ang_saida = (ang_chegada + 180) % 360
            st.caption(f"Saída a {ang_saida}° (tangente). Trações diferentes → há resultante.")
    ...
    # Transformação pela altura
    fator = alt_est / altura_util if altura_util > 0 else 1.0
    fx_t = fx * fator
    fy_t = fy * fator
    mag_t = mag * fator
    ...
    c3.metric("🔴 Resultante (daN)",  f"{mag_t:.1f}")
...
# Regra Electro:
#   Desconto topo = 0,20 m
#   Engastamento  = (altura × 10%) + 0,60 m
#   Altura útil   = altura − 0,20 − engastamento
_engastamento     = round(altura_poste * 0.10 + 0.60, 2)
_altura_util_auto = round(altura_poste - 0.20 - _engastamento, 2)

with c3:
    st.markdown("**Altura útil (automática)**")
    st.caption(
        f"Topo: −0,20 m | Engastamento: −{_engastamento:.2f} m "
        f"({int(altura_poste)}m × 10% + 0,60)"
    )
    altura_util = st.number_input(
        "Altura útil (m)", 1.0, altura_poste,
        value=_altura_util_auto, step=0.01,
        help="Calculado automaticamente. Edite só se necessário."
    )
    st.caption(f"✅ Altura útil padrão: **{_altura_util_auto:.2f} m**")
...
with st.container(border=True):
    fx1, fy1, mag1 = painel_nivel("1º Nível", "n1", altura_util, altura_util, altura_poste)
...
if tem_n2:
    with st.container(border=True):
        fx2, fy2, mag2 = painel_nivel(
            "2º Nível", "n2",
            max(0.0, altura_util - 1.5), altura_util, altura_poste
        )
...
if tem_sec:
    with st.container(border=True):
        fx_s, fy_s, mag_s = painel_nivel(
            "Secundária BT", "sec",
            max(0.0, altura_util - 3.0), altura_util, altura_poste,
            is_bt=True
        )
...
st.divider()
st.caption("Cálculos conforme DIS-NOR-014 Rev. 2024. Resultados devem ser validados por engenheiro responsável.")